...
{
	// Refuse Operation in PROShell Launcher
	if(sceKernelFindModuleByName("PROShell") != NULL) return;
	
	// Refuse Operation in yMenu Launcher
	if(sceKernelFindModuleByName("yMenu") != NULL) return;
	
	// Refuse Operation in vMenu Launcher
	if(sceKernelFindModuleByName("vmenu") != NULL) return;
	...
	// Trigger Reboot
	sctrlKernelLoadExecVSHWithApitype(0x141, VSH_PATH, &param);
}
