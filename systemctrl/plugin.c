...
#define PLUGIN_CONFIG_PATH (GAME_SAVE_PATH "plugins.txt")
...
static int matchingRunlevel(char * runlevel)
{
	...
	// "game" Runlevel
	if(apitype == 0x141 && stricmp(runlevel, "game") == 0) return 1;
...
static char * strtrim(char * text)
{
	...
	while(text[pos] != 0 && !isspace(text[pos])) pos++;
...
{
	// Open Plugin Config
	int fd = sceIoOpen(PLUGIN_CONFIG_PATH, PSP_O_RDONLY, 0777);
