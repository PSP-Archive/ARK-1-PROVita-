...
static sso_header sso_info;
static sso_part * sso_table = NULL;
static char sso_path[64];
static SceUID part_fd;
static int g_is_siso = 0;

static int siso_get_nsector()
{
	return sso_info.total_sectors;
};

static int is_siso(SceUID fd, const char * path)
{
	//read siso header
	sceIoLseek(fd, 0, PSP_SEEK_SET);
	sceIoRead(fd, &sso_info, sizeof(sso_header));

	if(sso_info.magic == SISO_MAGIC)
	{
		if(sso_table)
			oe_free(sso_table);
		
		//store siso path
		strcpy(sso_path, path);
	
		//create siso table
		sso_table = (sso_part *)oe_malloc(sizeof(sso_part) * sso_info.total_parts);
		
		//read siso parts entries
		sceIoRead(fd, sso_table, sizeof(sso_part) * sso_info.total_parts);
...
static int read_sso_data(unsigned char * addr, unsigned int size, unsigned int offset)
{
	unsigned absolute_start, absolute_end;	
	int part = -1, i;
	
	//search the part that holds the desired offset
	for(i = 0; i <= sso_info.total_parts; i++)
	{
		//"full" iso offsets for this part
		absolute_start = sso_table[i].sector_start * ISO_SECTOR_SIZE;
		absolute_end = sso_table[i].sector_end * ISO_SECTOR_SIZE;
		
		//desired offset is inside this part?
	...
	{
		//reset absolutes
		absolute_start = sso_table[part].sector_start * ISO_SECTOR_SIZE;
		absolute_end = sso_table[part].sector_end * ISO_SECTOR_SIZE;
		
		//build and open new part
		sprintf(strrchr(sso_path, '.') + 1, "%03d", part); //ms0:/ISO/myiso.XXX
		sceIoClose(part_fd);
		part_fd = sceIoOpen(sso_path, 0x000F0001, 0777);
		
		int retries = 0;
		int seek;
		do
		{
			seek = sceIoLseek(part_fd, offset - absolute_start, SEEK_SET);
			if(seek < 0)
				isoOpen();
			
			retries++;
		} while(seek < 0 && retries < 16); //retry until successfull seek
		...
		//calculate how much bytes to read
		if(offset + size < absolute_end)
			read_size = size;
		else
			read_size = absolute_end - offset;
		
		//read bytes
		retries = 0;
		do
		{
			ret = sceIoRead(part_fd, addr, read_size);
			if(ret < 0)
				isoOpen();
				
			retries++;
		} while(ret < 0 && retries < 16); //retry 
...
#ifdef USE_SISO
		g_is_siso = is_siso(g_iso_fd, g_iso_fn);
#endif
...
int isoRead(struct IoReadArg *args)
{
	...
	{
		ret = read_sso_data(args->address, args->size, args->offset);
	}
#endif
	else
	{
