...
void fixExploitGameModule(SceModule2 * mod)
{
	...
	if(sctrlKernelGetGameID(gameid) == 0 && strcmp(gameid, GAME_STRING) == 0)
	{
		// User Module
		if((mod->text_addr & 0x80000000) == 0)
		{
			...
			unsigned int i = 0; for(; i < mod->nsegment; i++)
			{
				...
				for(; base < (char *)mod->segmentaddr[i] + mod->segmentsize[i] - strlen(GAME_STRING); base++)
				{
					// Found Game ID
					if(strncmp(base, GAME_STRING, strlen(GAME_STRING)) == 0)
					{
						// Patch Game ID
						memcpy(base, "CB", 2);
